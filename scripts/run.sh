...
if [[ -e $validator_vote_account ]]; then
  ...
  renec-keygen new --no-passphrase -so "$validator_vote_account"
fi
...
if [[ -e $validator_stake_account ]]; then
  ...
  renec-keygen new --no-passphrase -so "$validator_stake_account"
fi
